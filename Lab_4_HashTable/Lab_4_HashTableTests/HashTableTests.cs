namespace Lab_4_HashTable
{
    public class MyHashTableTests
    {
        ...
        public void Test_ResizingStrategies()
        {
            ...
            TestResizingStrategy(hashTableArrayDoubleSize, "Dubbla storleken när belastningsfaktorn överstiger tröskelvärdet");
            
            var hashTableArrayIncremental = new MyHashTableArray<string, Student>();
            TestResizingStrategy(hashTableArrayIncremental, "Inkrementell ökning av arrayens storlek med en fast mängd");
            
            var hashTableArrayDynamic = new MyHashTableArray<string, Student>();
            TestResizingStrategy(hashTableArrayDynamic, "Dynamisk ändring baserad på statistisk analys");
        }

        private void TestResizingStrategy(IHashTable<string, Student> hashTable, string strategy)
        {
            // Lägg till nyckel-värde-par tills ändringar utlöses
            for (int i = 0; i < 15; i++)
            {
                hashTable.Add($"key{i}", new Student($"ID{i}", $"Namn{i}"));
            }

            // Kontrollera om alla värden kan hämtas
            for (int i = 0; i < 15; i++)
            {
                Assert.Equal($"Namn{i}", hashTable.Get($"key{i}").name);
            }

            Console.WriteLine($"Strategi: {strategy} - Testet lyckades.");
        }
    }
}
